...
By default, this module will register a client and dev only plugin that will display Largest Contentful Paint hints that will allow you to have a better Core Web Vitals score. You can however enable this functionality globally by setting the `hints` configuration globally like following (useful for CI and staging environments):

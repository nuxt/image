...
<img
  width="400"
  srcset="/_ipx/w_400/nuxt.png 1x, /_ipx/w_800/nuxt.png 2x"
>
```
By default (`densities="x1 x2"`), on devices with a device pixel ratio (DPR) of 1, `/_ipx/w_400/nuxt.png` will be used, and on devices with a DPR of 2 or higher, `/_ipx/w_800/nuxt.png` will be used.  

In this case, the `width` property is important. If you provide neither `width` nor `height`, the two generated image URLs will be the same, which is meaningless.

#### By media query and densities

If the image will be a different size on different window sizes, you may use `<NuxtImg>` like this:
...
On devices with a device pixel ratio (DPR) of 1, if `screenWidth <= 1024`, `/_ipx/w_300/nuxt.png` will be used, otherwise `/_ipx/w_700/nuxt.png` will be used.  

On devices with a DPR of 2, if `screenWidth <= 1024`, `/_ipx/w_600/nuxt.png` will be used (300 x 2), otherwise `/_ipx/w_1400/nuxt.png` will be used (700 x 2).

...
The default behavior is desktop-first. It compiles to `(max-width: ...px)` syntax, and sorts breakpoints in ascending order.

If you prefer a mobile-first strategy, you can set it in `nuxt-config.js`: `strategy: 'mobile-first'`.  
This will compile to `(min-width: ...px)` syntax, and sort breakpoints in descending order.

The default size is inferred depending on the strategy: the lowest breakpoint in mobile-first, the biggest breakpoint in desktop-first.

If a size doesn't have a screen prefix, it is removed from the sizes.

Each size generates an `srcset`, so if you want your image to be optimized for the most screens, you should provide a size for each breakpoint (even if the image width remains the same):
```html
<nuxt-img src="/logos/nuxt.png" />
<!-- will generate 1 image -->

<nuxt-img
  src="/logos/nuxt.png"
  sizes="sm:100vw md:100vw lg:100vw xl:100vw xxl:100vw"
  />
<!-- will generate 5 images -->
```

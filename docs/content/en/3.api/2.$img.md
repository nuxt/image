...
menuTitle: $img
title: $img
description: 'Using $img programmatically'
category: API
---

Sometimes it's necessary to apply image transformation programatically. You can use `$img` utility in context to generate image URLs for any purpose not possible with `<nuxt-img>` and `<nuxt-picture>` components.

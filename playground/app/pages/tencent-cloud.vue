<template>
  <div class="page">
    <header class="hero">
      <div>
        <p class="eyebrow">Tencent Cloud · COS</p>
    ...
    <section class="card-grid">
      <div class="card">
        ...
        <div class="field">
          ...
          <input v-model="baseURL" placeholder="https://<bucket>.cos.<region>.myqcloud.com" />
        </div>
        <div class="field">
          <label>Object Path (src)</label>
          <input v-model="src" placeholder="/path/to/image.jpg" />
        </div>
        <div class="field split">
          ...
          <div>
            <label>Height</label>
            <input v-model.number="height" type="number" min="0" />
          </div>
        </div>
        <div class="field split">
          <div>
        ...
        <div class="field split">
          ...
          <div>
            <label>blur</label>
            <input v-model.number="blur" type="number" min="0" max="50" />
          </div>
        </div>
        <div class="field split">
          <div>
            <label>rotate (deg)</label>
            <input v-model.number="rotate" type="number" min="0" max="360" />
          </div>
          <div>
            <label>sharpen</label>
            <input v-model.number="sharpen" type="number" min="0" />
          </div>
        </div>
        <div class="field split">
          <div>
            <label>
              <input type="checkbox" v-model="strip" /> strip (remove EXIF)
            </label>
          </div>
          <div>
            <label>
              <input type="checkbox" v-model="autoOrient" /> autoOrient
            </label>
          </div>
          <div>
            <label>
              <input type="checkbox" v-model="interlace" /> interlace
            </label>
          </div>
        </div>
        <button class="primary" @click="refreshKey++">Refresh Preview</button>
        <p class="hint">Make sure baseURL + src is publicly accessible (public read or behind CDN).</p>
      </div>
      ...
      <div class="card">
        ...
        <NuxtImg
          ...
          class="preview"
        />
        <p class="url" v-if="optimizedUrl">{{ optimizedUrl }}</p>
      </div>
      ...
      <div class="card">
        ...
        <img
          ...
          loading="lazy"
        />
        <p class="url" v-if="rawUrl">{{ rawUrl }}</p>
      </div>
    </section>
  </div>
</template>
...
const baseURL = ref('https://deploy-nuxt-on-edgeone-s9dwqjnz.edgeone.dev')
const src = ref('/tencentcloud/1.png')

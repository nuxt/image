...
describe("Renders simple image", () => {
  ...
  it("props.src is picked up by getImage()", () => {
    [
      ["source", "srcset", "/_ipx/f_webp&s_500x500/image.png"],
      ["img", "src"],
    ].forEach(([element, attribute, customSrc]) => {
  ...
  it("props.src is reactive", async () => {
    ...
    await nextTick();
    [
      ["source", "srcset", "/_ipx/f_webp&s_500x500/image.jpeg"],
      ["img", "src"],
    ].forEach(([element, attribute, src]) => {
      const domSrc = wrapper.find(element).element.getAttribute(attribute);
  ...
  it("renders src when svg is passed", () => {
    ...
    expect(wrapper.html()).toMatchInlineSnapshot(
      '"<picture><img data-nuxt-pic=\\"\\" src=\\"/image.svg\\"></picture>"'
    );
  });
